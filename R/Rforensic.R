...
# install DNAtools package
if(! "DNAtools" %in% rownames(installed.packages())) {
  install.packages("DNAtools", repos = "http://cran.rstudio.com/")
}
...
ObsMat <- function(grp, dat) {
  ...
  table(factor(dat[, cols[1]], levels = Names[Names > 0]),
	factor(dat[, cols[2]], levels = Names[Names > 0]))
}

ObsGen <- function(dat, Names) {
  Obsdf <- lapply(Names, ObsMat, data.frame(dat))
  ObsdfD <- lapply(Obsdf, function(tab) 
		   array(as.double(tab), dim = dim(tab), 
...
while(L <= nloci) {
  ...
  PIC <- 1 - sum((obs / sum(obs))^2) - 
         ( (sum((obs / sum(obs))^2)^2) + (sum((obs / sum(obs))^4)) )
  vecPIC[L] <- PIC 
  ...
  vecf[L] <- ( sum(numer1) + sum(numer2) / (2 * n) ) /
             ( sum(denom1) - sum(denom2) / (2 * n) )
...
for(i in seq_along(dfall)) {
  names(dfall[[i]]) <- c("alleles", paste0("freq.", i))
}

alfreqdf <- Reduce(function(x, y) 
		   merge(x, y, by = "alleles", all = TRUE), dfall)
...
pars <- rbind(nind, round(vech, 6), round(vech.exp, 6), round(vecf, 6), 
	      round(vecpM, 6), round(vecpD, 6), round(vecpE, 6), 
	      round(vecTPI, 6), round(vecPIC, 6))
rownames(pars) <- NULL
...
for(i in seq(2, ncol(pop), 2)) {
  ...
  m <- dim(pop[is.na(pop[, i]) | is.na(pop[, i+1]), ])[1] / 
       dim(pop[, c(i, i+1)])[1]
  missing <- c(missing, m)
}
...
barplot(missing * 100, space = 0.1, ylim = c(0, 100), axisnames = TRUE,
	col = "grey20", cex.names = 1.2, las = 2, names.arg = names(missing),
	border = NA, axis.lty = "solid", ylab = "missing data percentage (%)")
dev.off()
