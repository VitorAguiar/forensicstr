...
ObsGen = function(dat,Names) {
	...
	ObsdfD = lapply(Obsdf, function(tab) 
		array(as.double(tab),dim=dim(tab),dimnames=dimnames(tab)))
	names(ObsdfD) = Names
	return(ObsdfD)
}
...
pars = rbind(nind,round(vech,6),round(vech.exp,6),round(vecf,6),round(vecpM,6),
	     round(vecpD,6),round(vecpE,6),round(vecTPI,6),round(vecPIC,6))
rownames(pars) = NULL
